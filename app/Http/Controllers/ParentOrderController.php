...
abstract class ParentOrderController extends Controller
{
     ...
     */
    public function __construct(OrderRepository $orderRepo, ProductOrderRepository $productOrderRepository, CartRepository $cartRepo, PaymentRepository $paymentRepo, NotificationRepository $notificationRepo, MarketRepository $marketRepo, UserRepository $userRepository, OrderStatusRepository $orderStatusRepository, CouponRepository $couponRepository)
    {
        parent::__construct();
    ...
    protected function calculateDeliveryFee(): void
    {
        try {
            ...
            $market = $this->marketRepository->findWithoutFail($carts[0]->product->market->id);
            if($this->total < $market->mini_order)
                $this->order->delivery_fee = $market->delivery_fee;
